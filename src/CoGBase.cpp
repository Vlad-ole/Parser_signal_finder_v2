...
CoGBase::CoGBase(std::vector<double> &num_of_pe_in_event_vec, bool is_cog)
{
	...
	else
	{
		...
		for (int i = 0; i < num_of_pe_in_event_vec.size(); i++)
		{		
			if (num_of_pe_in_event_vec[i] > max_val)
			{
				max_val = num_of_pe_in_event_vec[i];
		...
		{
			int ch = GetChIdSiPMCorrect(array_index_of_max);
			for (int j = 0; j < ChCharacteristics::GetChCharacteristics().size(); j++)
			{
				if (ChCharacteristics::GetChCharacteristics()[j].ch_id == ch)
				{
				...
				if ( j == (ChCharacteristics::GetChCharacteristics().size() - 1) )
				{
					cout << "error" << endl;
					system("pause");
				}
